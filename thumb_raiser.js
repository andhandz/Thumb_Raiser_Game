...
export default class ThumbRaiser {
    constructor(mazeParameters, playerParameters, lightsParameters, fogParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters) {
        ...
        /* Turn on shadows in the renderer and filter shadow maps using the Percentage-Closer Filtering (PCF) algorithm */
    ...
    keyChange(event, state) {
        ...
        if (document.activeElement == document.body) {
            ...
            /* Toggle the user interface visibility
        ...
        if (!this.gameRunning) {
            ...
            if (!this.animations.actionInProgress) {
                ...
                if (this.maze.foundExit(this.player.position)) {
                    ...
                    /* Compute the distance covered by the player
                    ...
                    /* Compute the player's direction increment
                    ...
                    if (this.player.keyStates.run) {
                        /* Adjust the distance covered by the player
...
*/                  
                        ...
                        /* Adjust the player's direction increment
                            - now assume that the player is running:
                            - multiply the direction increment by this.player.runningFactor */
                        directionIncrement*=this.player.runningFactor;
                    }
                    /* Check if the player is turning left or right and update the player direction accordingly by adding or subtracting the direction increment
                    ...
                    /* Check if the player is moving backward or forward and update the player position accordingly
                    ...
                    if (this.player.keyStates.backward) { // The player is moving backward
                        const newPosition = new THREE.Vector3(-(coveredDistance*Math.sin(direction)), this.player.position.y, -(coveredDistance*Math.cos(direction))).add(this.player.position);
                        /* If the player collided with a wall, then trigger the death action; else, trigger either the walking or the running action
                            - death action: "Death"
                    ...
                    /* If the player is not moving nor emoting, then trigger the idle action
                        - idle ation: "Idle"
            ...
            /* If the fog is enabled, then assign it to the scene; else, assign null
